...
import { useCreateOrdem } from "@/hooks/use-ordens";
...
const novaOrdemSchema = z.object({
  descricao: z.string().min(1, "Descrição é obrigatória"),
  status: z.enum(["ABERTA", "EM_ANDAMENTO", "AGUARDANDO_PECAS"]).default("ABERTA"),
...
type NovaOrdemFormData = z.infer<typeof novaOrdemSchema>;
...
export function NovaOrdemModal({ isOpen, onClose }: NovaOrdemModalProps) {
  const [veiculosFiltrados, setVeiculosFiltrados] = useState<any[]>([]);
  const { toast } = useToast();
  const { createOrdem, isCreating } = useCreateOrdem();
  ...
  const previousClienteId = useRef<string | undefined>();
  ...
  // Reset veículo quando cliente muda (usando ref para evitar loops)
  useEffect(() => {
    if (clienteId && previousClienteId.current !== clienteId) {
      setValue("veiculoId", "");
      previousClienteId.current = clienteId;
    }
  }, [clienteId, setValue]);

  const onSubmit = async (data: NovaOrdemFormData) => {
    try {
